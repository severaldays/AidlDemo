...
### 7.1 核心决策：LLM只部署在云端 ☁️

| 决策 | 说明 |
|------|------|
| **车端不部署LLM** | 简化车端架构，降低资源占用 |
| **云端统一LLM服务** | 所有NLU和对话生成走云端 |
| **车端轻量NLU** | 本地只做意图分类和槽位提取 |
...
| 组件 | 技术 | 模型大小 | 用途 |
|------|------|----------|------|
| **意图分类** | TFLite + BERT-tiny | ~20MB | 快速意图识别 |
| **槽位提取** | 规则 + 正则 | 无模型 | 提取温度、位置等 |
| **关键词匹配** | Trie树 | <1MB | 车控指令快速匹配 |

**车端可识别的意图（离线）**:
```
车控类: turn_on_ac, set_temperature, adjust_seat, open_window...
查询类: query_weather, query_time, query_vehicle_status...
服务类: order_coffee, play_music, navigate_to...
```
...
│  │                          无LLM部署                                   │   │
│  │                                                                      │   │
│  │  开发语言: Kotlin (主) + C++ (音频处理)                             │   │
│  │  Agent框架: 自研Kotlin框架 (参考LangChain设计)                       │   │
│  │  NLU: TFLite意图分类 + 规则槽位提取 (轻量级,<50MB)                  │   │
│  │  语音: Porcupine(唤醒) + Vosk(离线ASR) + 讯飞(在线)                 │   │
│  │  存储: Room + MMKV                                                   │   │
│  │  通信: AIDL(车控) + OkHttp(云端) + EventBus(组件间)                 │   │
│  │                                                                      │   │
│  │  ❌ 不部署: LLM大模型 / Python / LangChain                          │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                           云端 (LLM集中部署)                         │   │
...
│  │  LLM服务: 通义千问API / GPT-4 API (无需自建推理)                     │   │
...
| 决策点 | 车端 | 云端 | 理由 |
|--------|------|------|------|
| **LLM部署** | ❌ 不部署 | ✅ 云端API | 简化车端，降低成本 |
| **开发语言** | Kotlin | Python | 车端原生，云端AI生态 |
| **Agent框架** | 自研Kotlin | LangChain | 车端无法运行Python |
| **NLU** | TFLite小模型 | LLM | 车端快速分类，云端深度理解 |
| **通信** | AIDL | REST/gRPC | 车端IPC，云端标准协议 |
